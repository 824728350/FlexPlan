...
'''
Generate mapping between synthesis and verification SMT variables
'''
    ...
    while line:
        if (".hdr_ParserImpl_" in line or "metas__parse_and_run" in line) and "version.version" not in line and ("___0" in line or "___" not in line) and "declare" in line:
            ...
            if "___0" in line:
                ...
                if "elements" in line:
                    name_end = find_nth(line,"|",-1)
                else:
                    name_end = line.find(" ()")
            name = line[name_start:name_end]
            if "metas__parse_and_run" in line:
                prefix_meta = name[:find_nth(name,".",2)]
            else:
                prefix_parse = name[:find_nth(name,".",2)]
            name_pr = name[find_nth(name,".",2)+1:find_nth(name,"_",-1)]
            name_su = name[find_nth(name,"_",-1)+1:]
            #print(name)
            if "metas__parse_and_run" in line:
...
def main():
    #args = parse_args()
    #print("extract from solve.smt.")
    vardict1, prefix_meta1, prefix_parse1 = process_output("solve.smt")
    #print("extract from verify-incremental.smt.")
    vardict2, prefix_meta2, prefix_parse2 = process_output("verify-incremental.smt")
    mapping = defaultdict()
