...
public:
  InstrumentBackend(Options &options)
    ...
    printf("Instrument Backend: after init bug check\n");
...
public:
  ExpandBackend(Options &options)
    ...
    auto evaluator = new P4::EvaluatorPass(&refMap, &typeMap);
    printf("ExpandBackend: after evaluator\n");
    addPasses({new P4::ResolveReferences(&refMap),
               new P4::TypeInference(&refMap, &typeMap, false)});
    addPasses({new CreateAnalysisHelpers});
    printf("ExpandBackend: after add passes\n");
    passes.push_back(new RemoveV1Controls(&refMap, &typeMap));
    ...
    printf("ExpandBackend: after handle stacks\n");
    passes.push_back(evaluator);
    ...
    printf("ExpandBackend: after all inlines\n");
    passes.push_back(evaluator);
    passes.push_back(new P4::ToP4(&dump_to, false));
    printf("ExpandBackend: after to p4\n");
  }
};
...
int main(int argc, char **argv) {
  ...
  try {
	...
	printf("Main function: Instrument backend triggered!\n");
        be = new analysis::InstrumentBackend(options);
	printf("Main function: Instrument backend finished!\n");
      } else if (!options.expand_to.isNullOrEmpty()) {
	printf("Main function: Expand backend triggered!\n");
        be = new analysis::ExpandBackend(options);
        printf("Main function: Expand backend finished!\n");
      } else {
        BUG("");
      }
      be->addDebugHook(hook, true);
      printf("Main function: add debug hook successful!\n");
    ...
    if (program == nullptr || ::errorCount() > 0)
      return 1;
    printf("Main function: program is null pointer or not?\n");
